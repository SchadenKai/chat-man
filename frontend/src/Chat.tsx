...
import type { ToolCall, Message } from "./interface";
    ...
    const subscriber: AgentSubscriber = {
      ...
      onToolCallStartEvent: (event) => {
        ...
        const toolCall: ToolCall = {
          ...
          name: event.event.toolCallName || "unknown",
          args: "",
          status: "running",
        };
        currentToolCallsRef.current.set(event.event.toolCallId, toolCall);
      ...
      onToolCallArgsEvent: (event) => {
        console.log("Tool call arguments: ", event);
        const existingToolCall = currentToolCallsRef.current.get(
          event.event.toolCallId
        );
        if (existingToolCall) {
          existingToolCall.args += event.event.delta || "";
      ...
      onToolCallResultEvent: (event) => {
        console.log("Tool call results: ", event);
        const existingToolCall = currentToolCallsRef.current.get(
          event.event.toolCallId
        );
        if (existingToolCall) {
          existingToolCall.result = event.event.content || "";
    ...
    <div className="flex flex-col h-screen w-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      ...
      <div className="bg-gray-800 border-b border-gray-700 px-6 py-4 shadow-lg">
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Chatman</h1>
          </div>
        </div>
      </div>
